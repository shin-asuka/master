...
public class PassportService {
    ...
    public Map<String, Object> senEmailForPassword(User user) {
        ...
        if(teacher == null){
            return MapReturnUtils.responseFail("The is a not exits teacher!", this);
        }
    ...
    public Map<String,Object> updatePassword(Teacher teacher, String newpassword) {
        User user = this.userDao.findById(teacher.getId());
        if (user == null){
            return MapReturnUtils.responseFail("User is null,Id:"+teacher.getId(), this);
        }
        //解码64
        String strPwd = new String(Base64.getDecoder().decode(newpassword));
        if (StringUtils.isBlank(strPwd)) {
            return MapReturnUtils.responseFail("new password base64 is error ! "+newpassword, this);
        }
        //SHA256加密
        SHA256PasswordEncoder encoder = new SHA256PasswordEncoder();
        user.setPassword(encoder.encode(strPwd));
        ...
        int i = this.userDao.update(user);
        if (i <= 0) {
            return MapReturnUtils.responseFail("update user result is "+i+" , id:"+user.getId(), this);
        }
