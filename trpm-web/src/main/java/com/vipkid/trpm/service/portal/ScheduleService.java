...
public class ScheduleService {
	...
	public Map<TimePoint, List<TimeSlot>> scheduleTable(List<Date> daysOfWeek, String timezone,
		...
		Comparator<TimePoint> comparator = (first, second) -> Long.valueOf(first.getMillis() - second.getMillis())
				.intValue();
	...
	public Map<String, Map<String, Object>> getTeacherScheduleMap(long teacherId, Date fromTime, Date toTime,
			String timezone) {
		List<Map<String, Object>> teacherScheduleList = onlineClassDao.findByTeacherIdWithFromAndToTime(teacherId,
		...
		/* 查询需要显示的INVALID课程列表 */
		List<Map<String, Object>> invalidScheduleList = onlineClassDao.findInvalidBy(teacherId, fromTime, toTime,
				timezone);
		...
		if (canSetSchedule(teacher.getId(), scheduleDateTime)) {
			...
			if (CourseType.isPracticum(courseType)) {
				...
				synchronized (teacher) {
					...
					List<OnlineClass> tList = onlineClassDao.findByTeacherIdAndScheduleDateTime(teacher.getId(),
							minusHour);

					long count = tList
							.stream()
							.filter((o) -> o.getClassType() == ClassType.PRACTICUM)
	...
	public boolean canSetSchedule(long teacherId, Timestamp t) {
		...
		long count = tList.stream()
				.filter((o) -> ClassStatus.isBooked(o.getStatus()) || ClassStatus.isAvailable(o.getStatus())).count();
	...
	public List<String> get24HourClass(long teacherId, List<String> onlineClassIds) {
		...
		try {
			...
			String requestUrl = ApplicationConstant.TEACHER_24HOUR_URL
					+ "/api/service/public/24HourClass/filterByClass";
	...
	public boolean checkInOneHour(long onlineClassId) {
		OnlineClass onlineClass = onlineClassDao.findById(onlineClassId);
		if (null != onlineClass
				&& (onlineClass.getScheduledDateTime().getTime() - System.currentTimeMillis()) > 1 * 60 * 60 * 1000) {
			return false;
		}
		return true;
	}
