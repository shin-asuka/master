...
public class ClassroomsController extends AbstractPortalController {
    ...
    public String classrooms(HttpServletRequest request, HttpServletResponse response, Model model) {
        if (IS_PRODUCTION) {
            ...
            /* 当前显示的课程类型 */
            String courseType =
                            ServletRequestUtils.getStringParameter(request, "courseType", CourseType.MAJOR.toString());
    ...
    public String showReport(HttpServletRequest request, HttpServletResponse response, Model model) {
        ...
        if (serialNumber.startsWith("P")) {
            model.addAllAttributes(classroomsService.practicumReport(serialNumber, onlineClassId, lessonId, studentId,
                            scheduledTime));
            /* Major报告显示 */
        } else {
            model.addAllAttributes(classroomsService.doShowReport(serialNumber, onlineClassId, lessonId, studentId,
                            scheduledTime));
        }
