...
`web_link` varchar(2048) NULL COMMENT '报告结果 web地址',
`pdf_link` varchar(2048) NULL COMMENT '报告结果 pdf地址',
...
`type` varchar(100) NULL COMMENT '报告的类型',
...
ADD COLUMN `screening_id` bigint(20) DEFAULT NULL COMMENT '背景调查表id' AFTER `teacher_application_id`;
