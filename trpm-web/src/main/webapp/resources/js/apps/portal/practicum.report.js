define([ "function", "jquery-bootstrap", "jquery-form", "jquery-load", "tools" ], function(report) {
	...
	/**定义错误函数处理*/
	var practicumReportError = function(reponse, status, info) {
	...
	/**页面请求成功*/
	var practicumReportSucceed = function(datas) {
	...
	/**页面按钮动画1*/
	var practicumReportAnimateOver = function() {
		$(this).next().css({
			"background" : "#eee",
			"color" : "#f00",
	...
	/**页面按钮动画2*/
	var practicumReportAnimateOut = function() {
		$(this).next().removeAttr("style");
	};

	/**页面请求开始*/
	var showPracticumReport = function(serialNumber, studentId, onlineClassId) {
		Portal.loading("open");
		var url = webPath + "/uploadPracticum.shtml";
		...
		$.ajaxRequest({
			...
			success : function(datas) {
				practicumReportSucceed(datas);
			},
			timeout : _timeout,
			error : function(reponse, status, info) {
				practicumReportError(reponse, status, info);
			}
		});
	};
	
	/**Report上传处理错误*/
	var practicumReportUploadError = function(reponse, status, info) {
	...
	/**Report上传进度处理*/
	var practicumReportUploadProgress = function(event, position, total, percentComplete) {
	...
	/**Report上传成功处理*/
	var practicumReportUploadSucceed = function(datas) {
		Portal.loading("close");
		$(".modal-body").hide();
		...
		if (datas.result) {
			...
			$("#successful").show();
			/* 上传成功后更新列表显示状态 */
			$("#" + _onlineClassId + " a").html("Prac Eval Form");
		} else {
	...
	/**Report上传提交之前*/
	var practicumReportBeforeSubmit = function() {
		Portal.loading("open");
	...
	/**Report上传提交*/
	var practicumReportSubmit = function() {
		...
		if(score == "" || score == "0"){
			message += "Please enter the practicum eval report score first!<br/>";
		}else{
			/**输入整数判断*/
			var d = /^[0-9]\d*$/i;
			if(!d.test(score) || score.indexOf("0") == 0){
				...
				message += "Scores should range from 1 to 44!<br/>";
			}
		}
		if(message != ""){
		...
		$("#uploadReportForm").ajaxSubmit({
			dataType : "json",
			data : data,
			beforeSubmit : practicumReportBeforeSubmit,
			success : function(datas) {
				practicumReportUploadSucceed(datas);
			},
			timeout : _timeout,
			uploadProgress : function(event, position, total, percentComplete) {
				practicumReportUploadProgress(event, position, total, percentComplete);
			},
			error : function(reponse, status, info) {
				practicumReportUploadError(reponse, status, info);
			},
	...
	var listener = function() {
		/** 文件特效 */
		$("#uploadReport").hover(practicumReportAnimateOver, practicumReportAnimateOut);
		...
		/* 上传逻辑实现 */
		$("#uploadReportFormSubmit").click(practicumReportSubmit);
	};

	return {
		showPracticumReport : showPracticumReport
	}
});
