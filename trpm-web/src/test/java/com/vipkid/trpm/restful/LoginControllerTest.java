...
public class LoginControllerTest extends BaseTestCase{
    ...
    public void resetPasswordRequest() throws IOException{
        ...
        response = Jsoup.connect(url).data(data).ignoreContentType(true).method(Method.POST).execute();
        json = response.body();
    ...
    public void auth() throws IOException{
        ...
        Response response = Jsoup.connect(url).ignoreContentType(true).header("token", "wertyuidjfkjshfldskjfl").method(Method.POST).execute();
        String json = response.body();
        logger.info(" auth OK ==> " + json);
        
        response = Jsoup.connect(url).ignoreContentType(true).method(Method.POST).execute();
        json = response.body();
